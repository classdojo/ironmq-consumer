...
(function() {
  var Consumer, DEFAULT_ADMIN_PORT, DEFAULT_OPTIONS, ErrorJournal, IronMQ, IronMQStub, Queue, jobLog, loopLog, restify, _, _auth, _setupAdmin;
    ...
    */
    ...
    Consumer.prototype._loop = function() {
      ...
      return this.queue.get({
        ...
        if (err) {
          return _this.__errorJournals.system.add(new Date().toISOString(), err);
          ...
          return jobs.forEach(function(job) {
            ...
            type = job.body.type;
            if (!type) {
              return _this.queue.error(job, new Error("Job Id " + job.id + " has no `type` field"));
            } else {
      ...
      return this.__q.get(options, function(error, messages) {
        ...
        for (_i = 0, _len = messages.length; _i < _len; _i += 1) {
          ...
          try {
            ...
            e = _error;
            _this.__errorJournal.add(message.id, new Error("Bad json data for message: " + message.body));
          }
        }
